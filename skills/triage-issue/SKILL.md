...
- **Pick the heading to match what actually happened, not a fixed template.** If something's genuinely done, it goes under "What's done" / "What remains". If a fix was tried and didn't work, or the reply is reporting a real blocker, don't file that under "What remains" — give it its own **Problem** / **Blocked on** heading stating what failed and why, so the reader isn't misled into thinking it's just an open follow-up.
